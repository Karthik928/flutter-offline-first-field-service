...
class _EmployeeVisitGroup {
  final String employeeId;
  final String firstName;
  final String lastName;
  final String empCode;
  final String profilePhoto;
  final List<Map<String, dynamic>> visits; // dealer OR farmer visits

  const _EmployeeVisitGroup({
...
// ─────────────────────────────────────────────────────────────────────────────
// Screen — Dealer Visits
// ─────────────────────────────────────────────────────────────────────────────

class ZonalDealerVisitsScreen extends StatefulWidget {
  const ZonalDealerVisitsScreen({super.key});

  @override
  State<ZonalDealerVisitsScreen> createState() =>
      _ZonalDealerVisitsScreenState();
}

class _ZonalDealerVisitsScreenState extends State<ZonalDealerVisitsScreen>
  ...
  // ── Flattened list of all dealer visits (with employee ref) ────────────────
  List<_FlatVisit> _flatVisits = [];
  ...
  Future<void> _fetchVisits() async {
    ...
    try {
      ...
      final List<_EmployeeVisitGroup> groups = [];
      final List<_FlatVisit> flat = [];

      for (final item in dataList) {
        if (item is! Map<String, dynamic>) continue;
        final emp = item['employee'] as Map<String, dynamic>? ?? {};
        final dealerVisits =
            (item['dealerVisits'] as List<dynamic>?)
                ?.cast<Map<String, dynamic>>() ??
            [];

        if (dealerVisits.isEmpty) continue;

        final group = _EmployeeVisitGroup(
        ...
        for (final v in dealerVisits) {
          flat.add(_FlatVisit(group: group, visitData: v));
        }
      }
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.store_outlined, size: 40, color: _accent),
          ),
          const SizedBox(height: 16),
          const Text(
          ...
          Text(
            'Dealer visits made by your team\nwill appear here.',
            textAlign: TextAlign.center,
    ...
    return FadeTransition(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return _DealerVisitCard(flatVisit: _flatVisits[index], index: index);
        },
      ),
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shared flat visit model (used by both dealer + farmer screens internally)
// ─────────────────────────────────────────────────────────────────────────────

class _FlatVisit {
  final _EmployeeVisitGroup group;
  final Map<String, dynamic> visitData;

  const _FlatVisit({required this.group, required this.visitData});
...
class _DealerVisitCard extends StatefulWidget {
  final _FlatVisit flatVisit;
  final int index;

  const _DealerVisitCard({required this.flatVisit, required this.index});

  @override
  State<_DealerVisitCard> createState() => _DealerVisitCardState();
}

class _DealerVisitCardState extends State<_DealerVisitCard>
    with SingleTickerProviderStateMixin {
  static const _accent = Color(0xFF1AB69C);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // ── Stats row: orders / payments / tickets ────────
                    if (hasDetails) ...[
                      const SizedBox(height: 12),
                      Row(
                        children: [
...
class _EmployeeRow extends StatelessWidget {
  final _EmployeeVisitGroup group;
  const _EmployeeRow({required this.group});
